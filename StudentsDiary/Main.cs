...
namespace StudentsDiary
{
    public partial class Main : Form
    {
        ...
        private void SetColumnHeader()
        {
            ...
            dgvDiary.Columns[6].HeaderText = "Technika";
        ...
        {
            var students = _fileHelper.DeserializeFromFile();
            if (cboFilter.SelectedItem.ToString() == "Wszyscy")
                RefreshDiary();
            else
            {
                var group = students.Where(x => x.GroupId == cboFilter.SelectedItem.ToString()).ToList();
                dgvDiary.DataSource = group;
            }
        }
    }
}
